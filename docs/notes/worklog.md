...
| Session 15 — storage health (D26) + editor truncation — **partially closed 2026-07-22** | **Hot-plug/retry-forever: closed as a formality.** The 3D.14 Pico 1 pass explicitly confirmed "SD/PSRAM diag health + hot-eject/reinsert recovery (D26 retry-forever): works as expected" — DET-pin polling and the retry heartbeat are shared-mainboard logic (identical hardware whichever Pico module is socketed), so this covers both boards. **Still genuinely open, not yet checked on either board:** the specific Y=-editor visual detail (store a regression model to y1, open Y= — text should end `...` before the checkbox with no overlap) and the red-`SD`-indicator-in-the-status-bar visual (vs. just "recovers cleanly" functionally) weren't distinctly probed during 3D.14's interview — worth a quick look next time either board is in hand, low priority |

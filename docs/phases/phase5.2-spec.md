...
| P5.2-7 | Should the unified evaluator replace tinyexpr on the numeric path too? | **Raised 2026-08-09 by P5.2-6; DEFERRED past 5.2 closure (D50).** §5.2's "would double arithmetic cost" argues against a `Complex` numeric path, not against this evaluator, which keeps a real tier — so the guardrail's stated reason does not transfer. Four costs do: a fixed 2,064 B `Program` vs a malloc'd tree (~120 B for `sin(x)+2*x`); `compile()`/`run()` are non-reentrant singletons and the numeric path re-enters; the `kCompute` arena invariant (`stats`/`matrix`/`infer` own it); and no differential corpus off the home screen. Gains: F1 fixed everywhere, four parsers → one, tinyexpr's depth-7 cap gone from graphing, 7,897 B of flash. ~~**The missing input is §9's M1**~~ — **supplied 2026-08-09 by 5.2.12's M6 apportionment, and it argues AGAINST replacing.** `listops::seq` compiles once and evaluates many, i.e. the graphing shape; measured there the VM is **~1.7x slower per operation** than tinyexpr's compiled tree walk, plus ~4.6 us/element of fixed re-entry. M1's one-shot win (-22 to -32%) comes from removing the REAL-mode double evaluation and does not transfer to repeated evaluation. The measurement supports the split that already exists. **D50's amendment carries the projection onto the real workload**: ~1.75x on every numeric-path consumer (`FunctionSource` per pixel column, `TableModel` per row, `numeric_solve` per iteration, `fnInt` per sample) — seven Y= slots at three ops go **86.6 -> 152.3 ms per redraw**, on the screen D10 leg B already exists to speed up. Two corrections to the costs listed above: **M2 does not transfer** and must not be counted here, because `evaluate_real()` is scalar-only and has no list broadcasting to regress; and the **14.4 KB `Program` figure is avoidable** by not caching (compile is a fixed ~0.19 ms against ~12 ms of evaluation). Flash would *improve*: -7,897 B returned against the +2-3 KB this phase spent. |
